    ...
    for attempt in range(retries):
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as resp:
                ...
                # Bybit v5 uses retCode=0 for success
                if ret_code is not None and ret_code != 0:
                    log_error("API", f"{path} retCode={ret_code} msg={data.get('retMsg')}")
                    if attempt < retries - 1:
                        time.sleep(2 ** attempt)
                        continue
                    # Return data anyway on last attempt (some endpoints use retCode differently)
                    return data
                return data
...
def get_usdt_perpetual_pairs():
    """Get all USDT perpetual trading pairs. Handles pagination via cursor."""
    pairs = []
    cursor = ""
    ...
    while True:
        ...
        data = _request("GET", "/v5/market/instruments-info", params)
        if not data:
            log_error("PAIRS", "No response from instruments-info endpoint")
            break

        result = data.get("result")
        if result is None:
            log_error("PAIRS", f"No 'result' key in response. Keys: {list(data.keys())}")
            break
...
def get_positions():
    """Get all open positions. Returns list or None on failure."""
    data = _request("GET", "/v5/position/list",
                    {"category": "linear", "settleCoin": "USDT", "limit": "200"}, auth=True)
    if data is None:
        return None
    if "result" not in data:
        log_error("POS", f"Unexpected response: {list(data.keys())}")
        return None
