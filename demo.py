                ...
                for p in self.positions:
                    if not p.closed:
                        state["positions"].append({
                            ...
                            "sl_roi": p.sl_roi,
                            "tp_roi": p.tp_roi,
                            "reversal_detected": p.reversal_detected
                        })
    ...
    def _load_state(self):
        ...
        try:
            ...
            for pd in state.get("positions", []):
                ...
                pos.reversal_detected = pd.get("reversal_detected", False)
            ...
            if (pos.side == "LONG" and new_direction == "SHORT") or \
               (pos.side == "SHORT" and new_direction == "LONG"):
                ...
                log("REVERSAL", f"{symbol} | Strategy now {new_direction} vs open {pos.side} | "
                    f"Waiting for +3% to +5% ROI before close")
                self._save_state()
        ...
        while self._running:
            try:
                ...
                for pos in active:
                    ...
                    if getattr(pos, 'reversal_detected', False):
                        if roi >= 3:  # minimum +3% ROI before smart closing
                            self._close_position(pos, price, "SMART CLOSE (reversal)")
                            continue
