...
def scan_pairs(pairs, trader, usd_php):
    ...
    for symbol in ordered_pairs:
        ...
        if trader.has_position(symbol):
            # Still analyze the pair to detect strategy reversal
            try:
                candles_5m = exchange.get_klines(symbol, config.ENTRY_TF, 100)
                candles_1h = exchange.get_klines(symbol, config.PRIMARY_TF, 200)
                if len(candles_5m) >= 100 and len(candles_1h) >= 200:
                    ...
                    if sig["direction"] and sig["score"] >= config.SIGNAL_MIN_SCORE:
                        # Check if direction conflicts with open position
