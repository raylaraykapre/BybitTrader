...
class Position:
    ...
    def __init__(self, symbol, side, entry_price, qty, leverage, usd_php):
        self.symbol = symbol
...
class LiveTrader:
    ...
    def open_position(self, symbol, direction, balance_usdt):
        ...
        tp_roi = config.TAKE_PROFIT_ROI_PCT  # e.g. 350 (positive)
        ...
        pos = Position(symbol, direction, price, qty, leverage, self.usd_php)
        with self.lock:
            self.positions.append(pos)

        entry_php = price * self.usd_php
        log("ORDER", f"{direction} {symbol} | Entry:₱{entry_php:,.0f} "
            f"SL:{config.STOP_LOSS_ROI_PCT}% TP:+{config.TAKE_PROFIT_ROI_PCT}% "
            f"Lev:{leverage}x")
            ...
            with self.lock:
                for pos in self.positions:
                    if not pos.closed and pos.symbol not in open_symbols:
                        ...
                        elif roi >= config.TAKE_PROFIT_ROI_PCT:
